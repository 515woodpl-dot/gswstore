...
export default async function SalesPage({ searchParams }: Props) {
  ...
  const [{ data: orders }, { data: inventory }] = await Promise.all([
    sb
    .from("orders")
    .select("id,order_number,created_at,total,discount_total,status,source,sold_by_name,transaction_type,internal_use_reason,walk_in_customer_id,order_items(id,item_id,name,sku,quantity,unit_price,list_price,cost_price,discount_amount,discount_reason)")
